...
## Tuned Information (by mambo)

1. add function to find minimum value [in target_space.py](code/target_space.py)
2. add Acquisition function to apply entropy search method [in util.py](code/util.py)


## memo purpose about .py file

- util
   1. def acq_max : find Maximum value 
   2. class UtilityFunction : manange to Acquistion function
   3. def load_logs : load previos data
   4. def ensure_rng : create random number
   5. class Colours : manange graph's color
- target_space
   1. def _hashable : change array element to float
   2. class TargetSpace : manage parameter space and target value
-  observer
   1. class Observer : abstract class
   2. class _Tracker : remembers time and the current maximum value & update current maximum value
- logger
   1. ScreenLogger : using observer._Tracker
   2. JSONLogger : manage json file using observer._Tracker
- Event
   1. Start, Step, End
- bayesian_optimization
   1. class Queue : manage to data sequence
   2. class Observable : choose data input type --> subscribe, unsubscribe, find data --> dispatch
   3. class BayesianOptimization : manage all of bayesian optimization's object
